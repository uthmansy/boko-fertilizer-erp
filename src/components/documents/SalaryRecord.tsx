...
interface SalaryRecordProps {
  ...
  data: EmployeePayrollJoined[];
}

const SalaryRecord: React.FC<SalaryRecordProps> = ({ data, payroll }) => {
  return (
    <Document>
          ...
          <View>
            <View style={styles.horizontalTable}>
              <View style={styles.horizontalRow}>
                <Text style={{ ...styles.label, width: 30 }}>SN</Text>
              ...
              {data.map((payroll, i) => (
                <View key={i} style={styles.horizontalRow}>
                  <Text style={{ ...styles.value, width: 30 }}>{i + 1}</Text>
                  <Text style={styles.value}>
                    {payroll.employee.first_name} {payroll.employee.last_name}
                  </Text>
                  <Text style={styles.value}>
                    N{formatNumber(payroll.to_be_paid)}
                  </Text>

                  <Text style={styles.value}>
                    {
                      //@ts-ignore
                      `N${formatNumber(payroll.net_pay)}`
                    }
                  </Text>
                  <Text style={styles.value}>{payroll.employee.bank_name}</Text>
                  <Text style={styles.value}>{payroll.note}</Text>
                </View>
              ))}
            </View>
          </View>
